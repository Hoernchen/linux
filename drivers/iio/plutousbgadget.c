...
static int
iiousb_bind(struct usb_configuration *c, struct usb_function *f)
{
...
static void
iiousb_free_func(struct usb_function *f)
{
...
static void work_func_enq(struct work_struct *work){
  ...
  do {
        ...
        ret = wait_event_interruptible(mydata.tx_wq, ((req = take_req(&mydata.tx_idle, &mydata.tx_lock)) ));
        if (!req) {
            break;
        }
...
//        }
        ...
        if(mydata.enabled != 1)
            usb_ep_free_request(mydata.in_ep, req);
...
static void work_func_tx(struct work_struct *work)
{
    ...
    do {
        if (!buffer->access->data_available(buffer)) {
            ...
            if (ret || mydata.ad9361_iiodev->info == NULL )
                return;
        }
        else
            printk("nowait\n");
        ...
        //printk("in %s:%d l:%.8x a:%.8x %d\n", __FUNCTION__, __LINE__, blockreqs[localblk.id].req->length, blockreqs[localblk.id].req->buf, blockreqs[localblk.id].req->context);
...
DBGTRACE
    ...
    for(i =0; i < NUMBLK; i++)
        usb_ep_dequeue(ep, blockreqs[i].req);

    VDBG(cdev, "%s disabled\n", mydata.function.name);
}

static int
enable_iiousb(struct usb_composite_dev *cdev, struct plutousb *ss,
        int alt)
{
    int result = 0, i=0;
    struct usb_ep *ep = mydata.in_ep;
...
static int iiousb_set_alt(struct usb_function *f,
...
static void work_func_cmd(struct work_struct *work){
  ...
  do {
        ...
        ret = wait_event_interruptible(mydata.cmd_wq, ((req = take_req(&mydata.cmd_idle, &mydata.cmd_lock)) ));
        if (!req) {
            break;
        }
        if(req->status != 0)
...
//        DBGTRACE
        ...
        case 0x01: // ad9361_phy_store
        {
            ...
            for(; *a != NULL; a++ ){
                struct iio_dev_attr *this_attr = to_iio_dev_attr(*a);
                if( strcmp(reqdata, this_attr->dev_attr.attr.name) == 0){
                    if(this_attr->dev_attr.store)
                        this_attr->dev_attr.store(&mydata.ad9361_phydev->dev, &this_attr->dev_attr, beginvalue, strlen(beginvalue));
                    printk("phy:%s %s\n", this_attr->dev_attr.attr.name, beginvalue);
                    break;
                }
            }
        }
        ...
        case 0x02: // chan 1 RX_LO
        {
            ...
            for (; ext_info->name; ext_info++){
                if( strcmp(reqdata, ext_info->name) == 0){
                    if(ext_info->write)
                        ext_info->write(mydata.ad9361_phydev, ext_info->private, d, beginvalue, strlen(beginvalue));
                    printk("rxlo: %s %s\n", ext_info->name, beginvalue);
                    break;
                }
            }
        }
        ...
        case 0x03: //chan 4 RX1 manual gain
        {
            ...
            mydata.ad9361_phydev->info->write_raw(mydata.ad9361_phydev, d, gain_db, gain_mdb, IIO_CHAN_INFO_HARDWAREGAIN);
            printk("hwgain: %d %d\n", gain_db, gain_mdb);
        }
        break;
        ...
        case 0x04: //chan 4 RX1 sample freq
        {
            ...
            mydata.ad9361_phydev->info->write_raw(mydata.ad9361_phydev, d, sfreq, 0, IIO_CHAN_INFO_SAMP_FREQ);
        }
        break;
...
static int iiousb_setup(struct usb_function *f,
...
static bool iiousb_rqmatch(struct usb_function *a, const struct usb_ctrlrequest *ctrl, bool config0){
...
{
    struct iiousb_opts *this_opts;
    this_opts = container_of(to_config_group(item), struct iiousb_opts, func_inst.group);
...
static ssize_t iiousb_buf_num_store(struct config_item *item,
...
// supply a match function to prevent the other gadgets (i.e. mass storage) stop eating our ctrl req
static int iio_dev_node_match(struct device *dev, void *data)
{
    struct iio_dev *testdev = dev_to_iio_dev(dev);

    if(testdev && testdev->name){
        if (strcmp(testdev->name, data) == 0)
            return 1;
    }
    return 0;
}
...
static int __init iiousb_modinit(void)
{
    ...
    DBGTRACE
    mydata.ad9361_iiodev = dev_to_iio_dev(bus_find_device(&iio_bus_type, NULL, "cf-ad9361-lpc", iio_dev_node_match));
    mydata.ad9361_phydev = dev_to_iio_dev(bus_find_device(&iio_bus_type, NULL, "ad9361-phy", iio_dev_node_match));
